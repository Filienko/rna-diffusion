...
import torch.optim
import random
from src.classification.large_dim.utils import confusion_matrix, compute_auc, EarlyStopping
...
class TissuePredictor():
    ...
    def __init__(self, config:dict=None, dataset:str=None):
        ...
        self.model = self.Classifier(self.input_dim, self.config['hidden_dim1'], self.config['hidden_dim2'], self.nb_classes, self.config['dropout'])
    ...
    def load_model(self, path=None, device=None):
        ...
        try:
            ...
            print(f"No previously trained model at given path: {path}. Please train classifier using 'train' function first or correct path")
    ...
    class Classifier(nn.Module):
        ...
        def __init__(self, input_dim, hidden_dim, hidden_dim2, output_dim, dropout_ratio=0.5):
            ...
            self.block1 = nn.Sequential(self.proj1, 
                                        nn.ReLU(), 
