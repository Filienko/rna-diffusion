...
for i in range(5):
    ...
    x_fake= x_fake.to(CONFIG_MLP['device']).float() # to device
    ...
    if i==0:
        ...
        pd.DataFrame(data=x_fake, columns=np.concatenate((landmark_genes_id, non_landmark_genes_id))).to_csv(f'./src/reconstruction/results/fake_from_vae_reconstructed_mlp_{DATASET}.csv')
        pd.DataFrame(data=y_fake.cpu().numpy().argmax(1).reshape(-1,1), columns=['tissue_type']).to_csv(f'./src/reconstruction/results/fake_from_vae_reconstructed_mlp_{DATASET}_labels.csv')
...
df_res.to_csv(f'./src/reconstruction/results/results_mlp_{DATASET}_from_vae.csv', index=False)
print(f"----> End. ")