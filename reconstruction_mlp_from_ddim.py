...
def main():
    ...
    CONFIG_MLP['device'] = CONFIG.device2
    ...
    for i in range(5):
        ...
        x_fake= torch.from_numpy(x_fake).to(CONFIG_MLP['device']).float()
        ...
        if i==0:
            ...
            pd.DataFrame(data=x_fake, columns=np.concatenate((landmark_genes_id, non_landmark_genes_id))).to_csv(f'./src/reconstruction/results/fake_from_ddim_reconstructed_mlp_{DATASET}.csv')
            pd.DataFrame(data=y_fake.argmax(1).reshape(-1,1), columns=['tissue_type']).to_csv(f'./src/reconstruction/results/fake_from_ddim_reconstructed_mlp_{DATASET}_labels.csv')
    ...
    df_res.to_csv(f'./src/reconstruction/results/results_mlp_{DATASET}_from_ddim.csv', index=False)
    print(f"----> End. ")
