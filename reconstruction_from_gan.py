...
for i in range(5):
    ...
    if i==0:
        ...
        pd.DataFrame(data=x_fake, columns=np.concatenate((landmark_genes_id, non_landmark_genes_id))).to_csv(f'./src/reconstruction/results/fake_from_gan_reconstructed_lr_{DATASET}.csv')
        pd.DataFrame(data=y_fake.cpu().numpy().argmax(1).reshape(-1,1), columns=['tissue_type']).to_csv(f'./src/reconstruction/results/fake_from_gan_reconstructed_lr_{DATASET}_labels.csv')
...
df_res.to_csv(f'./src/reconstruction/results/results_lr_{DATASET}_from_gan.csv', index=False)
print(f"----> End. ")